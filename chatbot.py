...
class ChatInterface(Frame):
    ...
    def __init__(self, master=None):
        ...
        help_option.add_command(label="About Rihanna", command=self.msg)
        ...
        status = ttk.Progressbar(self.entry_frame, orient=HORIZONTAL, length=180, mode='indeterminate')
    ...
    @staticmethod
    def msg():
        tkinter.messagebox.showinfo("Rihanna v1.0",
                                    'Rihanna is a chatbot developed for fun')

    @staticmethod
    def about():
        tkinter.messagebox.showinfo("Rihanna Developers",
                                    "Emeka")
    ...
    def chat(self, msg):
        theme = {"Torque": self.color_theme_turquoise, "Hacker": self.color_theme_hacker,
                 "Blue": self.color_theme_dark_blue,
                 "Grey": self.color_theme_grey, "Default": self.color_theme_default}
        msg = msg.lower().strip()
        if msg[:len('change theme ')] == 'change theme ':
            th = msg[len('change theme '):].capitalize()
            if th in theme:
                theme[th]()
                return f'Theme changed to {th}'
            else:
                return f'cannot find theme {th}'
        else:
            return str(rihanna(msg)['say'])
